...
def process_video(video_path, video_id):
    """Process the video for accident detection and save up to 10 frames."""
    ...
    m = video_process3.Mo(fps)
    while True:
        ret, frame = cap.read()
        if not ret:
            break
        frame, is_acc, is_car_stop = m.process(frame)
        ...
        processing_status[video_id] = {
            ...
            'is_accident': is_acc,        # Include accident status
            'is_car_stopped': is_car_stop # Include car stop status
        }
...
@app.route('/')
def index():
    """Display the main page with buttons for OCR or Accident Detection."""
    return render_template('index.html')

@app.route('/choice/<option>')
def choice(option):
    """Handle user choice and redirect to the respective upload page."""
    if option == 'ocr':
        return redirect(url_for('ocr_page'))
    elif option == 'accident_detection':
        return redirect(url_for('upload_page'))
    else:
        return 'Invalid choice', 404
...
def ocr_page():
    """Handles both GET and POST requests for the OCR page."""
    
    if request.method == 'POST':
        # Handle POST request (file upload and OCR processing)
        ...
        # Perform object detection or OCR on the saved file
        text_list = object_detection(path_save, filename)
        print(text_list)

        # Render the template with the uploaded image and OCR results
        return render_template('ocr.html', upload=True, upload_image=filename, text=text_list, no=len(text_list))
    
    # Handle GET request (simply display the OCR page without any upload)
...
@app.route('/upload')
def upload_page():
    """Page for uploading a video for accident detection."""
